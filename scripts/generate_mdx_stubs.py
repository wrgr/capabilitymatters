...
For every YAML record in landscape/resources/*/ of a wrgr/lecommons checkout
(set LECOMMONS_DIR, default ../lecommons) that carries `featured: true`, check
whether an MDX file under src/content/ already references it via
...
# MDX type formats (from src/content.config.ts); we map content_type onto them.
...
def iter_featured_yaml(res_dir: Path) -> list[tuple[Path, dict]]:
...
def main() -> int:
    ...
    res_dir = lecommons_dir() / "landscape" / "resources"
    existing = load_existing_refs()
    featured = iter_featured_yaml(res_dir)
