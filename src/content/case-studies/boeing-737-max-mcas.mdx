...
The 737 MAX is Therac-25 written at the scale of an airline fleet. To sell the jet without costly new pilot training, Boeing masked its changed handling with software — MCAS — then left it out of the manuals and let it fire on a single failing sensor. When that sensor failed, crews who had never heard of the system could not recover, and 346 people died. Five investigations converged on one diagnosis: the training omission wasn't an oversight, it was a contract deliverable — human capability removed to avoid the cost of sustaining it, waved through by a certification regime too close to Boeing to catch it.
